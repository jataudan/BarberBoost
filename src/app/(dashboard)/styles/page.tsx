      ...
      <div className="relative w-full max-w-lg bg-[#111111] border border-white/[0.06] rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-2">
            ...
            {imageUrl ? (
              <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-[#1a1a1a]">
                ...
                <button type="button"
                  onClick={() => { setImageUrl(''); setUploadErr(null) }}
        ...
        <div className="px-5 py-4 border-t border-white/[0.06] flex-shrink-0">
          <button type="button" onClick={handleSave} disabled={saving || uploading}
            ...
            {saving ? <><Loader2 className="w-4 h-4 animate-spin" />{isEdit ? 'Saving…' : 'Adding…'}</>
...
export default function StylesPage() {
  ...
  useEffect(() => {
    if (!shopId) return
    void fetchStyles()
    // Also load staff for barber-assignment UI
  ...
  function openEdit(style: HaircutStyle) {
    setEditStyle(style)
    setModalOpen(true)
  }
  ...
  async function handleToggle(id: string, active: boolean) {
    ...
    if (res.ok) {
      const j = await res.json() as { data?: HaircutStyle }
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-zinc-600 mt-1">
              Add haircut reference photos so clients can pick their desired style when booking.
            </p>
          </div>
          <button
            type="button"
