...
export function IosInstallModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Save ${shopName} to your phone`}
      ...
      <div className="relative z-10 w-full sm:max-w-sm mx-auto bg-[#111111] border border-white/[0.08] rounded-t-3xl sm:rounded-3xl shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3">
          <div className="flex items-center gap-3">
            {logoUrl ? (
              <img src={logoUrl} alt={`${shopName} logo`} className="w-10 h-10 rounded-xl object-cover" />
            ) : (
              <div
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-500 mt-0.5">
                {isIos ? 'Save to iPhone Home Screen' : 'Save to Android Home Screen'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-5 py-5 space-y-4">
          <p className="text-xs text-zinc-400 leading-relaxed">
            To save <span className="text-white font-semibold">{shopName}</span> to your home screen:
          </p>
...
function Step({ number, accentColor, children }: { number: number; accentColor: string; children: React.ReactNode }) {
  return (
...
function ShareIcon({ accentColor }: { accentColor: string }) {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
      stroke={accentColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
