...
export function BookingModal({
  ...
  // ── Step renderers ────────────────────────────────────────────────
  const steps: Record<Step, { title: string; content: React.ReactNode }> = {
    1: {
      title: 'Date & Barber',
      content: (
      ...
      content: (
        <div className="space-y-4">
          <p className="text-xs text-zinc-500">
            {selectedStaff?.name} · {format(parseISO(selectedDate), 'd MMMM yyyy')}
        ...
        return (
          <div className="space-y-4">
            <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 space-y-2.5">
              <SummaryRow icon={<Calendar className="w-3.5 h-3.5" />} label={format(parseISO(selectedDate), 'EEEE, d MMMM yyyy')} />
